#!/usr/bin/env python3
"""Build a fully self-contained Caption Inspector v7.5.app.
...
APP_NAME = "Caption Inspector v7.5"
BUNDLE_ID = "com.local.captioninspector.v75"
APP_VERSION = "7.5"
...
def copy_app_sources(resources):
    ...
    # A module added to python/ but not to APP_MODULES ships a bundle that
    # imports fine in the working tree and fails on the user's machine. The list
    # stays explicit - not everything in python/ belongs in the app - but it has
    # to be checked against reality.
    present = {path.name for path in (REPO_ROOT / "python").glob("*.py")}
    missing = sorted(present - set(APP_MODULES))
    if missing:
        raise SystemExit(
            "These modules exist in python/ but are not listed in APP_MODULES, so they "
            "would be left out of the bundle:\n"
            + "\n".join(f"  {name}" for name in missing)
            + "\n\nAdd them to APP_MODULES in packaging/build_offline_app.py, or delete "
...
def find_app_icon():
    """Locate the app icon without hard-coding a version.

    It lives in whichever launcher bundle is at the repo root, and in every
    frozen snapshot. Naming a specific one means the icon quietly disappears at
    the next version bump.
    """
    candidates = sorted(REPO_ROOT.glob("*.app")) + sorted(
        REPO_ROOT.glob("zDeprecated/*/*.app"), reverse=True
    )
...
def build(output, models, thin, wheel_dir, skip_sign, force=False):
    ...
    if app.exists():
        ...
        if not marker.exists() and not force:
            raise SystemExit(
                ...
                "  python3 packaging/build_offline_app.py --output ~/Desktop/'Caption Inspector v7.5.app'\n"
    ...
    if icon_source:
        ...
        log("warning: no AppIcon.icns found; the bundle will use the generic icon")
