...
The Windows equivalent has to be built **on a Windows machine** — nothing here
cross-compiles from macOS or Linux. It uses PyInstaller instead of hand-relocating
a framework, since Windows has no `install_name_tool`/`codesign` step to design
around; ffmpeg's and libci's DLL dependencies are still walked and vendored
explicitly, the same way `vendor_ffmpeg` does it for macOS.

```
windows\Build-OfflineApp-Windows.cmd
:: or directly
py -3 packaging\build_offline_app_windows.py --models base
```

Prerequisites on the Windows build machine: Python 3.12 from python.org (with
tcl/tk) on PATH as `py`, and MSYS2 with the UCRT64 `clang`/`make`/`pkg-config`/
`ffmpeg` packages so `libci.1.0.0.dll` can be built and ffmpeg's DLLs vendored —
`windows\Install-And-Launch-CaptionInspector.ps1` sets both of those up.

Output: `packaging/dist/CaptionInspector/` (self-contained folder) and
`packaging/dist/Caption-Inspector-v7.5-windows-x64.zip` to hand to another
machine. See `packaging/windows_entry.py` for how the frozen `.exe` finds its
bundled `libci.dll`, ffmpeg, and models without touching the network or the
system PATH.

**This build path is new and has not been run end-to-end on a real Windows
machine yet** — treat the first run as a verification pass: confirm the `.exe`
launches, decodes a file of each type, and runs a transcription with the
machine's network disconnected before relying on the zip.

## macOS

```
make offline-app            # build with the base model
make offline-app MODELS=base,medium   # ...or with more than one
make verify-offline-app     # prove the result is self-contained
make dist                   # package it into a DMG + zip to send
make stage-model MODEL=medium         # add a model to an already-built bundle
```
...
A built bundle carries only the models it was built with, and it will not download
more — so **every model you want the app to offer has to be bundled**. The app shows
which ones are present in its picker and refuses the rest up front, but that is a
better error, not a substitute for shipping what people need.

| model | size | notes |
|---|---|---|
| `tiny` | 78 MB | fast, rough |
| `base` | 148 MB | the default; fine for sync measurement |
| `small` | 500 MB | |
| `medium` | 1.5 GB | noticeably better on noisy dialogue |
| `large-v3` | 2.9 GB | slow on CPU; rarely worth it for timing work |

Adding one to an existing bundle takes seconds and does not need a rebuild:

```
python3 packaging/stage_model.py --model medium --download "Caption Inspector v7.5.app"
```

That copies the weights in and **re-seals the bundle**, which is not optional: adding a
file to a signed bundle invalidates its signature, and macOS then kills the app on
launch with `SIGKILL` and no message. If the bundle was signed with a Developer ID, the
tool stops and asks for the same identity rather than silently downgrading it to ad-hoc
and breaking Gatekeeper for everyone you sent it to.
