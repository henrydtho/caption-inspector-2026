...
# Short cues - "Yeah.", "- Oh.", "It's massive." - carry too little to search the
# whole transcript with: "yeah" occurs in every other minute of dialogue. But
# once the cues either side of one are anchored, its position is boxed into a few
# seconds, and inside that box a single word is usually unambiguous. So they get
# their own pass, run last, searching only between their matched neighbours.
#
# Without it a programme of short reactions reports most of its lines as
# unmatchable: on a reality delivery, 21 of 25 sampled lines were under the
# three-token floor.
SHORT_CUE_RADIUS_SECONDS = 3.0
# Stricter than MIN_CONFIDENCE: one or two words is thin evidence, and a wrong
# anchor here would put a fabricated offset in a vendor report.
SHORT_CUE_MIN_CONFIDENCE = 0.80
...
class Match:
    ...
    def __init__(self, cue, audio_seconds, confidence, matched_text, start_offset=0.0):
        ...
        # True when anchored by the short-cue pass, from its neighbours rather
        # than from its own text.
        self.short_cue = False
...
def _short_cue_tokens(cue):
    """Tokens of a cue too short for the main pass, but not empty.

    A cue with no words at all is a sound effect and has nothing to match.
    """
    tokens = [token for token in cue.words if token]
    if not tokens or len(tokens) >= MIN_CUE_TOKENS:
        return []
    return tokens


def _match_short_cues(cues, index, matches, start_offset, cancel=None):
    """Second-chance pass for cues too short to search the transcript globally.

    Each is searched only between the audio positions of its nearest matched
    neighbours, so the ambiguity that made it unsearchable is removed by
    context rather than by lowering the bar.

    A cue with no anchored neighbour on either side is left alone: with nothing
    to bound the search there is no reason to trust a one-word hit.
    """
    if not matches:
        return []

    anchored = {id(match.cue): match for match in matches}
    ordered = list(cues)
    recovered = []

    for position, cue in enumerate(ordered):
        raise_if_cancelled(cancel)
        if id(cue) in anchored:
            continue

        tokens = _short_cue_tokens(cue)
        if not tokens:
            continue

        previous = next(
            (anchored[id(ordered[i])] for i in range(position - 1, -1, -1)
             if id(ordered[i]) in anchored),
            None,
        )
        following = next(
            (anchored[id(ordered[i])] for i in range(position + 1, len(ordered))
             if id(ordered[i]) in anchored),
            None,
        )
        ...
        # Where the cue should land, carried over from whichever neighbour is
        # closer in caption time.
        cue_seconds = cue.seconds - start_offset
        reference = previous or following
        predicted = cue_seconds + (reference.audio_seconds - reference.media_seconds)

        low_seconds = predicted - SHORT_CUE_RADIUS_SECONDS
        high_seconds = predicted + SHORT_CUE_RADIUS_SECONDS

        # Never search past an anchored neighbour: the cue cannot be spoken
        # before the line before it or after the line after it.
        if previous is not None:
            low_seconds = max(low_seconds, previous.audio_seconds)
        if following is not None:
            high_seconds = min(high_seconds, following.audio_seconds)
        if high_seconds <= low_seconds:
            continue

        low, high = index.index_range_for_time(low_seconds, high_seconds)
        found = _find_best_window(index, tokens, low, high)
        if not found or found["confidence"] < SHORT_CUE_MIN_CONFIDENCE:
            continue

        match = Match(
            cue, found["audio_seconds"], found["confidence"], found["text"], start_offset
        )
        match.short_cue = True
        recovered.append(match)
        anchored[id(cue)] = match
...
def align_cues_to_transcript(cues, transcript, start_offset=0.0, progress=None, cancel=None):
    ...
    # Run last, so it can use everything the fine pass anchored as context.
    recovered = _match_short_cues(cues, index, matches, start_offset, cancel)
    if recovered:
        if progress:
            progress(f"Recovered {len(recovered)} short cues from their neighbours.")
        matches.extend(recovered)
        matches.sort(key=lambda match: match.media_seconds)
