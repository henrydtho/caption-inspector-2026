...
from timecode import AUTO_RATE_LABEL, format_offset_ms, format_seconds, rate_choice_labels, rate_code_for_choice
...
class ComparePanel(ttk.Frame):
    def __init__(self, parent):
        ...
        # Only meaningful for an SCC too short to auto-detect a rate from -
        # see the Options row and `_update_rate_override`.
        self.rate_override_a = tk.StringVar(value=AUTO_RATE_LABEL)
        self.rate_override_b = tk.StringVar(value=AUTO_RATE_LABEL)
    ...
    def _build(self):
        ...
        self.rate_override_combo_a = ttk.Combobox(
            inputs,
            textvariable=self.rate_override_a,
            values=rate_choice_labels(),
            state="disabled",
            width=22,
        )
        self.rate_override_combo_a.grid(row=1, column=2, sticky="w")
        self.rate_override_combo_a.bind(
            "<<ComboboxSelected>>", lambda _event: self._show_rate(self.path_a.get().strip(), "A")
        )
        ...
        self.rate_override_combo_b = ttk.Combobox(
            inputs,
            textvariable=self.rate_override_b,
            values=rate_choice_labels(),
            state="disabled",
            width=22,
        )
        self.rate_override_combo_b.grid(row=3, column=2, sticky="w")
        self.rate_override_combo_b.bind(
            "<<ComboboxSelected>>", lambda _event: self._show_rate(self.path_b.get().strip(), "B")
        )
    ...
    def _show_rate(self, path, side):
        ...
        The rate override combobox only enables for `.scc`: it is the one
        format with no out-of-band rate and short enough timecodes to sometimes
        defeat auto-detection (see `frame_rate_detect.py`) - the same edge case
        the Inspect tab's frame rate dropdown exists for. Every other format
        either states its own rate or does not need one.
        """
        variable = self.rate_a_text if side == "A" else self.rate_b_text
        override_combo = self.rate_override_combo_a if side == "A" else self.rate_override_combo_b
        override_combo.config(state="readonly" if Path(path).suffix.lower() == ".scc" else "disabled")
        ...
        def work():
            try:
                result = compare_subtitles(
                    ...
                    cancel=token,
                    rate_code_a=rate_code_for_choice(self.rate_override_a.get()),
                    rate_code_b=rate_code_for_choice(self.rate_override_b.get()),
                )
