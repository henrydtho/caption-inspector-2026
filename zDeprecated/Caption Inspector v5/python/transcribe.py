...
from offline import apply_environment, model_is_available, resolve_model
...
def transcribe_media(
    ...
    if not model_is_available(model_size):
        available = ", ".join(MODEL_SIZES)
        raise TranscriptionError(
            f"The {model_size!r} model is not available locally, and this build does not "
            "download models.\n"
            f"Either choose a model this machine already has, or stage the weights for "
            f"{model_size!r} into the bundle's models directory.\n"
            f"Model sizes this app knows about: {available}."
        )
