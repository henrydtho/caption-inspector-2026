...
v4 is built to run with the network switched off, permanently. That is not a
...
#   Caption Inspector v4.app/Contents/Resources/
...
def model_is_available(model_size):
    ...
    Either the bundle carries it, or it is already in the Hugging Face cache.
    """
    root = models_root()
    if root is not None and resolve_model(model_size) != model_size:
        return True
    return _in_huggingface_cache(model_size)
