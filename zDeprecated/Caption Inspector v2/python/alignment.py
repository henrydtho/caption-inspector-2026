...
class Match:
    ...
    __slots__ = ("cue", "audio_seconds", "offset", "confidence", "matched_text")

    def __init__(self, cue, audio_seconds, confidence, matched_text):
        self.cue = cue
        self.audio_seconds = audio_seconds
        # Positive offset: the caption is on screen before the words are spoken.
        self.offset = audio_seconds - cue.seconds
...
def _find_best_window(index, tokens, low=None, high=None):
    ...
    return {
        ...
        "audio_seconds": index.starts[start],
        "text": candidate_text,
    }
...
def align_cues_to_transcript(cues, transcript, progress=None):
    ...
    for cue, tokens in usable[::sample_step]:
        ...
        if found:
            coarse_points.append((cue.seconds, found["audio_seconds"] - cue.seconds))
    ...
    matches = []
    for cue, tokens in usable:
        predicted = intercept + slope * cue.seconds
        centre = cue.seconds + predicted
        ...
        if not found:
            continue
        matches.append(Match(cue, found["audio_seconds"], found["confidence"], found["text"]))

    return matches


def _fit_drift(matches, duration=None):
    points = [(match.cue.seconds, match.offset) for match in matches]
    if len(points) < 2:
        return {
            "slope": 0.0,
...
def summarize_alignment(cues, matches, media=None):
