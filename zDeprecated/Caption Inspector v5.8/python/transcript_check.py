...
def build_transcript(
    ...
    short_recovered = sum(1 for match in matches if getattr(match, "short_cue", False))
    if short_recovered:
        notes.append(
            f"{short_recovered} short lines - one or two words, like \"Yeah.\" or \"- Oh.\" - "
            "were matched using the lines either side of them for context. On their own they "
            "are too common to locate in a transcript; between two anchored neighbours they "
            "are not."
        )
