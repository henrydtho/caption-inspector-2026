...
def _read_side(path, result, label):
    ...
    """
    try:
        document = read_comparable_document(path)
...
def compare_subtitles(path_a, path_b, tolerance_ms=DEFAULT_TOLERANCE_MS, cancel=None):
    ...
    raise_if_cancelled(cancel)
    result.file_a = _read_side(path_a, result, "File A")
    raise_if_cancelled(cancel)
    result.file_b = _read_side(path_b, result, "File B")
    if result.errors:
        return result
...
def render_html_report(result, app_version="5.8", max_differences=2000):
    if result.errors:
        rows = "".join(
...
def write_comparison_report(path, result, app_version="5.8"):
