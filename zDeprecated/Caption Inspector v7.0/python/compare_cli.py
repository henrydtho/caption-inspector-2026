...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "--rate-a",
        type=int,
        help="Explicit x100 frame rate for file A, for an SCC too short to auto-detect one from",
    )
    parser.add_argument(
        "--rate-b",
        type=int,
        help="Explicit x100 frame rate for file B, for an SCC too short to auto-detect one from",
    )
...
def main(argv=None):
    ...
    try:
        result = compare_subtitles(
            ...
            cancel=cancel,
            rate_code_a=args.rate_a,
            rate_code_b=args.rate_b,
        )
