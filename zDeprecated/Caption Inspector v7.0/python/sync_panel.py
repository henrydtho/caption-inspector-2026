"""The Sync QC tab of the v4 desktop app.
...
A third one matters once Tier 2 is running: Stop. Transcribing a feature takes
minutes, and the way out used to be quitting the app.
"""
...
class SyncQCPanel(ttk.Frame):
    ...
    def _build(self):
        ...
        self.model_picker = ModelPicker(options, self.model_size)
        self.model_picker.grid(row=1, column=1, sticky="w", padx=(8, 18), pady=(10, 0))
        ...
        # Disabled until something is actually running, so it is never a live
        ...
        self.report_button.grid(row=0, column=3, padx=(10, 0))

        self.install_button = ttk.Button(actions, text="Install faster-whisper", command=self._install_engine)
        self.install_button.grid(row=0, column=4, padx=(10, 0))
    ...
    def _run_full(self):
        ...
        options = self._snapshot_options()
        if not self.model_picker.is_ready():
            messagebox.showwarning("Sync QC", self.model_picker.explain())
            return
        ...
        def work():
            ...
            try:
                ...
                # Tier 1 already rendered, so keep it and say Tier 2 stopped.
                self.after(0, self._on_stopped, "Stopped during Tier 2. The Tier 1 result stands.")
                return
            except Exception as error:
                self.after(0, self._on_failure, str(error))
                return
    ...
    def _stop(self):
        """Ask the worker to stop. The token kills any live ffmpeg child too."""
    ...
    def _on_stopped(self, message="Stopped. No result was produced."):
