...
"""Package a built bundle into something you can actually send someone.
    ...
    python3 packaging/make_distributable.py "Caption Inspector v4.app"

Produces, in `packaging/dist/`:

    Caption Inspector v4-<version>-<arch>.dmg    what to send
    Caption Inspector v4-<version>-<arch>.zip    for anything that dislikes DMGs
...
To install: open the .dmg and drag Caption Inspector v4 to Applications.
...
def main(argv=None):
    ...
    app = Path(args.app) if args.app else REPO_ROOT / "Caption Inspector v4.app"
    app = app.resolve()
    if not app.exists():
        raise SystemExit(
            f"No bundle at {app}\n"
