...
# SRT/VTT/TTML count from the start of the program, and captions decoded out of
# a container are stamped from its PTS, which starts at zero.
PROGRAM_TIMECODE_SUFFIXES = (".scc", ".mcc")
...
def caption_timeline_offset(media, rate_code, caption_path):
    ...
    a 3510-second sync error. It applies to SCC and MCC only - subtracting the
    start timecode from a WebVTT file, whose zero is already the head of the
    program, would invent the same error in the other direction.
    """
