...
# Caption sources whose stamps are absolute program timecode, and so have to be
# rebased before they can be compared to anything measured from the head of the
# media file. Everything else is already on the media file's own timeline:
# SRT/VTT/TTML and the other text formats count from the start of the program,
# and captions decoded out of a container are stamped from its PTS, which starts
# at zero.
#
# Both STL flavours belong here - EBU-STL puts its first subtitle after the
# start-of-programme timecode in the GSI header, and Spruce STL is authored
# against the tape - but `.stl` is also the extension Spruce uses, so the
# decision is made on the parsed kind where one is available.
PROGRAM_TIMECODE_SUFFIXES = (".scc", ".mcc", ".stl")


def caption_timeline_offset(media, rate_code, caption_path):
    """Seconds to subtract from caption times to land on the media timeline.

    Returns (offset, timecode) so a caller can say *why* it shifted anything.

    This is the correction that keeps a 00:58:30:00 head from being reported as
    a 3510-second sync error. It applies to the timecode-stamped formats only -
    subtracting the start timecode from a WebVTT file, whose zero is already the
    head of the program, would invent the same error in the other direction.
    """
    if Path(caption_path).suffix.lower() not in PROGRAM_TIMECODE_SUFFIXES:
        return 0.0, None
    return _video_start_seconds(media, rate_code)
...
def tier1_check(caption_path, video_path=None, rate_code=None, tolerance_frames=2,
                count_frames=False, cancel=None):
    ...
    `cancel` is checked between checks. Tier 1 is usually too fast to stop, but
    `count_frames` decodes the whole file, and that is not.
    """
    ...
    if video_path:
        raise_if_cancelled(cancel)
        try:
            media = probe_media(video_path, count_frames=count_frames, cancel=cancel)
...
def tier2_check(
    ...
    `cancel` is honoured at every stage that can run long - the ffmpeg audio
    extraction, each transcribed segment, and the alignment loop - so Stop takes
    effect in about a second rather than at the end of the run.
    """
    from alignment import align_cues_to_transcript, describe_drift, summarize_alignment
    from caption_cues import extract_cues
    from transcribe import TranscriptionError, ensure_model_available, transcribe_media
    ...
    # Checked first: everything below - probing, cue extraction, audio
    # extraction - is wasted if the model cannot be loaded at the end of it.
    try:
    ...
    # SCC/MCC cues are stamped in absolute program timecode; the transcript is
    ...
    # Text subtitles and captions decoded from the container are already
    # media-relative, and `caption_timeline_offset` returns 0.0 for them.
    ...
    matches = align_cues_to_transcript(cues, transcript, start_offset=start_offset, cancel=cancel)
