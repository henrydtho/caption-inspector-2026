...
"""Package a built bundle into something you can actually send someone.
    ...
    python3 packaging/make_distributable.py "Caption Inspector v6.0.app"
...
will refuse to open it until you clear the download flag. Open Terminal and run:
    ...
    xattr -dr com.apple.quarantine "/Applications/Caption Inspector v6.0.app"
...
def main(argv=None):
    ...
    app = Path(args.app) if args.app else REPO_ROOT / "Caption Inspector v6.0.app"
    app = app.resolve()
    if not app.exists():
        raise SystemExit(
            f"No bundle at {app}\n"
