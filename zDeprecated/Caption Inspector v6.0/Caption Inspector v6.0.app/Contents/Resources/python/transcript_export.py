...
from alignment import MIN_CUE_TOKENS
...
UNMATCHED = "unmatched"  # searched for and genuinely not found
UNCHECKED = "unchecked"  # no video was supplied, so nothing was compared
NOT_DIALOGUE = "not_dialogue"  # a sound effect or music cue - nobody says it
TOO_SHORT = "too_short"        # too few words to anchor on; never attempted

# The last two exist because reporting them as "not found in audio" accuses the
# caption file of a fault the app never tested for. A programme is full of
# "[cheering]" and "- Oh." - on a real delivery they were 40% of the lines, and
# every one of them read as a failure.
STATUS_WORDS = {
    MATCHED: "in sync",
    OFF: "out of tolerance",
    UNMATCHED: "not found in audio",
    UNCHECKED: "not checked",
    NOT_DIALOGUE: "not dialogue",
    TOO_SHORT: "too short to match",
}

# Outcomes that say nothing about whether the file is in sync.
UNTESTABLE = (NOT_DIALOGUE, TOO_SHORT)
...
class TranscriptResult:
    ...
    @property
    def untestable(self):
        """Lines the matcher could not attempt: sound effects and one-worders."""
        return sum(1 for line in self.lines if line.status in UNTESTABLE)

    @property
    def testable(self):
        """Lines that carried enough dialogue to look for."""
        return len(self.lines) - self.untestable

    @property
    def match_rate(self):
        """Found, over what could be looked for.

        Measured against testable lines, not every line. A programme's sound
        effects are not evidence of anything, and counting them dragged the rate
        down on files that were fine.
        """
        if not self.lines or not self.checked or not self.testable:
            return None
        found = self.matched + self.out_of_tolerance
        return found / self.testable
    ...
    def verdict_sentence(self):
        ...
        if self.untestable:
            aside = (
                f" {self.untestable} further lines carry no dialogue to match "
                "(sound effects, or too few words); they were not checked either way."
            )

        if found == 0:
            if not self.testable:
                return (
                    "No line in this file carries enough dialogue to match against audio - "
                    "they are all sound effects or single words, so nothing could be "
                    "checked." + aside
                )
            return (
                f"None of the {self.testable} dialogue lines could be found in the video's "
                "audio. Either this caption file belongs to a different asset, or the audio "
                "has no usable speech." + aside
            )
        ...
        if self.out_of_tolerance == 0 and rate >= 0.5:
            return (
                ...
                "This transcript matches the video." + aside
            )
        return (
            ...
            f"{self.unmatched} of {self.testable} dialogue lines were not found in the audio."
            + aside
        )
...
def _strip_leading_speaker(text, speaker):
    """Remove the speaker label from the front of a caption line.

    Only from the front, and only the label this cue reported - a name appearing
    later in the line is dialogue about someone, not attribution.
    """
    ...
    if stripped.startswith(speaker):
        remainder = stripped[len(speaker):].lstrip()
...
def _untestable_reason(cue):
    """Why the matcher could not even attempt this cue, if it could not.

    Mirrors `alignment._anchor_tokens`, which silently declines anything under
    MIN_CUE_TOKENS. A cue it never looked at must not be reported as missing
    from the audio.
    """
    words = getattr(cue, "words", None)
    if words is None:
        words = (getattr(cue, "normalized", "") or "").split()

    if not words:
        return NOT_DIALOGUE
    if len(words) < MIN_CUE_TOKENS:
        return TOO_SHORT
    return None
...
def build_lines(cues, matches=None, tolerance_ms=200.0, checked=False):
    ...
    for index, cue in enumerate(cues, start=1):
        ...
        if not checked:
            status = UNCHECKED
        elif match is None and _untestable_reason(cue):
            status = _untestable_reason(cue)
        else:
            status = _status_for(match, tolerance_ms)

        # The speaker travels in its own field, and every renderer prints it
        # ahead of the text. Leaving the label in the text too printed it twice:
        # "LAUREN: LAUREN: What, the Cheshire grapevine?".
        speaker = getattr(cue, "speaker", None)
        text = cue.text
        if speaker:
            text = _strip_leading_speaker(text, speaker)
...
def _header_lines(result):
    lines = [
        ...
        f"Format: {format_label(result.source_kind) if result.source_kind else 'unknown'}",
    ...
    if result.checked:
        lines.append(
            f"Matched: {result.matched + result.out_of_tolerance} of {result.testable} "
            f"dialogue lines ({result.matched} in tolerance, {result.out_of_tolerance} out, "
            f"{result.unmatched} not found)"
        )
        if result.untestable:
            lines.append(
                f"Not checked: {result.untestable} lines with no dialogue to match "
                "(sound effects, or too few words)"
            )
...
def _annotation(line):
    if line.status == UNCHECKED:
        return ""
    if line.status in UNTESTABLE:
        return f"  [{STATUS_WORDS[line.status]}]"
...
_STATUS_CLASS = {
    MATCHED: "ok", OFF: "off", UNMATCHED: "missing", UNCHECKED: "none",
    NOT_DIALOGUE: "none", TOO_SHORT: "none",
}
