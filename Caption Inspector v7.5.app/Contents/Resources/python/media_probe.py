...
from offline import find_executable
...
def _run_cancellable(command, cancel=None):
    """Run a child process, killing it if the user presses Stop.

    `subprocess.run` cannot be interrupted, and an ffmpeg decode of a feature is
    minutes long, so the process is spawned and registered with the token. The
    token terminates it on cancel; we just wait and then notice.
    """
    if cancel is None:
    ...
    # A killed child looks like a failed child; the token says which it was.
    raise_if_cancelled(cancel)
    return process.returncode, stdout or "", stderr or ""


def ffprobe_path():
    """Bundled ffprobe if this is a packaged build, otherwise the one on PATH."""
...
def probe_media(path, count_frames=False, cancel=None):
    """Run ffprobe against `path`.

    `count_frames` decodes the whole file for an exact frame count. It is slow
    and off by default; the container count is right for normal deliveries, and
    it is the reason this takes a cancel token.
    """
    ...
    try:
        returncode, stdout, stderr = _run_cancellable(command, cancel)
    ...
    if returncode != 0:
        message = stderr.strip() or f"ffprobe exited with {returncode}"
...
def extract_audio(media_path, output_path, sample_rate=16000, stream_index=None,
    ...
    returncode, _stdout, stderr = _run_cancellable(command, cancel)
    if returncode != 0:
        message = stderr.strip() or f"ffmpeg exited with {returncode}"
