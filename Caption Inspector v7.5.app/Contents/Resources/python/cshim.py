...
def data_608_callback(time, line21code):
    ...
    if line21code.codeType == BASIC_NA_CHARACTERS or line21code.codeType == SPECIAL_NA_CHARACTERS \
        ...
        # `backgroundForgroundData` is misspelled in line21_decode.h, and the
        # ctypes struct above mirrors the header exactly, as it must. Spelling
        # it correctly here raised AttributeError inside a ctypes callback,
        # where Python prints "Exception ignored" and carries on - so every
        # mid-row code was silently dropped from the track instead of being
        # added below.
        if line21code.codeUnion.midRow.backgroundOrForeground == 0:  # Background Color
            midrow_code = MidRowControlCode(caption_time, line21code.codeUnion.midRow.chan,
                                            ...
                                            line21code.codeUnion.midRow.backgroundForgroundData.color, None)
        else:                                                        # Foreground Color
            midrow_code = MidRowControlCode(caption_time, line21code.codeUnion.midRow.chan,
                                            line21code.codeUnion.midRow.isPartiallyTransparent,
                                            line21code.codeUnion.midRow.isUnderlined, None,
                                            line21code.codeUnion.midRow.backgroundForgroundData.style)
